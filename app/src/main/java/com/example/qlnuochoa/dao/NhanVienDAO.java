package com.example.qlnuochoa.dao;
...
import com.example.qlnuochoa.database.DbHelper;
...
public class NhanVienDAO {
    ...
    public NhanVien getTen(String id){
        NhanVien  nv = new NhanVien();
        String[] args = new String[]{ id + ""};
        Cursor c = db.rawQuery("SELECT maNV, hoTenNV FROM NhanVien WHERE maNV=?", args);
        if(c.moveToFirst()){
            nv.maNV = c.getInt(0);
            nv.hoTenNV = c.getString(1);
        }
        return nv;
    }
//    tim ten Nhan Vien
